...
create_branch() {
    ...
    # LOCAL ONLY — no remote pulls (cloud actions disabled)
    log "  Using local main (no remote fetch)"
...
Co-Authored-By: Hermes <hermes@aycrith.dev>"
    ...
    # LOCAL ONLY — commit directly to main after validation
    # No push, no PRs — GitHub Actions disabled, all validation is local
    git checkout main
    git merge --no-ff "$branch" -m "autonomous: $task (local)"
    git branch --delete "$branch"

    success "Merged $branch to main locally (no push — cloud actions disabled)"
}
