...
# =============================================================================
class GitManager:
    """Manages branches, commits for autonomous work. LOCAL ONLY — no push/PR."""

    def __init__(self, project_root: Path):
        self.root = project_root
        # No GitHub token — all work is local, cloud actions disabled
    ...
    def create_branch(self, task: ImprovementTask) -> str:
        ...
        # LOCAL ONLY — no remote fetch (cloud actions disabled)
    ...
    def commit_and_push(self, task: ImprovementTask, branch: str) -> bool:
        """Stage, commit, and merge to main locally. NO PUSH — cloud actions disabled."""
        # Check for changes
        ...
        # LOCAL ONLY — merge directly to main (no push, no PRs)
        run("git checkout main 2>/dev/null || git checkout master")
        run(f"git merge --no-ff {branch} -m \"autonomous: {task.title} (local)\"")
        run(f"git branch --delete {branch} 2>/dev/null || true")

        success("Changes committed and merged to main locally (no push)")
        return True

    def create_pr(self, task: ImprovementTask, branch: str) -> Optional[str]:
        """NO-OP — PRs disabled (cloud actions off, all work is local)."""
        warn("PR creation skipped — cloud actions disabled, work merged locally")
        return None
